...
class ApiController {
	...
	static setNewBalance(sum, isSpending = true) {
		...
		promiseSetNewBalance.then((val) => {
			if (val) {
				// Ensure val.balance is a number before arithmetic
				const currentBalance = Number(val.balance);
				const amountSpent = Number(sum);
				if (!isNaN(currentBalance) && !isNaN(amountSpent)) {
					ViewManipulator.updateBalance(currentBalance - amountSpent, currentBalance);
				} else {
					console.error('ApiController.setNewBalance - Error: val.balance or sum is not a number. val:', val, 'sum:', sum);
				}
			} else {
