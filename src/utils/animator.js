...
class Animator {
	...
	static animate({ duration, delta, step, delay }) {
		...
		const id = setInterval(function () {
			...
			const easedProgress = delta(progress); // Use a different name for the result

			step(easedProgress); // Pass the eased progress to the step function
	...
	static showWinningDialog(data) {
		// WinningDialog constructor expects an elementID, e.g., '#winningDialogContainer'
		// The data (winnerName, prizeAmount) is passed to the show() method.
		// Assuming the dialog ID is standard, or get it from config if it can vary.
		const dialogElementId = '#winningDialogContainer'; // Or Initializer.config.gameConf.winningDialog.elementID or similar
		const dialog = new WinningDialog(dialogElementId);
		dialog.show(data); // Pass the data to the show method
	}

	static showFlyingPrize(data) {
		const prize = new FlyingPrize(data);
		prize.show(); // Assuming a 'show' or 'animate' method
	}
}
